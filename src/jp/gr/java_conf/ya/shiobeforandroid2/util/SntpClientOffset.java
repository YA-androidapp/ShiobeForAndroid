 ...
 *
 * Sample usage:
 * <pre>SntpClientOffset client = new SntpClientOffset();
 * if (client.requestTime("time.foo.com")) {
 *     long now = client.getNtpTime() + SystemClock.elapsedRealtime() - client.getNtpTimeReference();
 * }
 * </pre>
 */
public final class SntpClientOffset {
	...
	/**
	 * Returns the offset of the NTP transaction
	 *
	 ...
	 */
	final long getNtpTimeReference() {
	 ...
	 */
	private final long read32(final byte[] buffer, final int offset) {
	 ...
	 */
	private final long readTimeStamp(final byte[] buffer, final int offset) {
	 ...
	 */
	public final boolean requestTime(final String host, final int timeout) {
	 ...
	 */
	private final void writeTimeStamp(final byte[] buffer, int offset, final long time) {
		final long seconds = ( time / 1000L ) + OFFSET_1900_TO_1970;
